    ...
    useEffect(() => {
        if (searchedQuery) {
            const filteredJobs = allJobs.filter((job) => {
                ...
                const salaryCondition = searchedQuery?.includes("below 5 lakh")
                    ? parseInt(searchedQuery.charAt(6)) * 100000 > job.salary * 100000
                    : false || searchedQuery?.includes("above 5 lakh")
                        ? parseInt(searchedQuery.charAt(6)) * 100000 <= job.salary * 100000
                        : false;

                return titleMatch || descriptionMatch || locationMatch || salaryCondition;
